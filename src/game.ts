...
const POLICE_BUMPER_CONTACT_GAP =
  PLAYER_HALF_LENGTH + COP_HALF_LENGTH - 0.18;
...
function trackBodyContact(
  playerProgress: number,
  playerLane: number,
  copProgress: number,
  copLane: number,
): TrackBodyContact | null {
  const longitudinalOverlap =
    PLAYER_HALF_LENGTH + COP_HALF_LENGTH -
    Math.abs(playerProgress - copProgress);
  const lateralOverlap =
    PLAYER_HALF_WIDTH + COP_HALF_WIDTH - Math.abs(playerLane - copLane);
  if (longitudinalOverlap <= 0 || lateralOverlap <= 0) return null;
  return {
    longitudinalOverlap,
    lateralOverlap,
    lateralDirection: playerLane >= copLane ? 1 : -1,
  };
}
...
export function startPursuitGame(
  ...
  const statusRef = {
    current: requiredElement<HTMLElement>("status-value"),
  };
  const assetStatus = requiredElement<HTMLDivElement>("asset-status");
  const loadingPercent = requiredElement<HTMLElement>("loading-percent");
  const loadingTrack = requiredElement<HTMLDivElement>("loading-track");
  const loadingFill = requiredElement<HTMLElement>("loading-fill");
  const knobRef = {
  ...
  const setLoadingProgress = (value: number, label: string) => {
    const percent = THREE.MathUtils.clamp(Math.round(value), 0, 100);
    loadingFill.style.width = `${percent}%`;
    loadingPercent.textContent = `${percent}%`;
    loadingTrack.setAttribute("aria-valuenow", String(percent));
    statusRef.current.textContent = label;
  };
  setLoadingProgress(4, "BUILDING PURSUIT…");
  ...
  const setPhase = (phase: Phase) => {
    ...
    const modelProgress = { player: 0, police: 0 };
    const updateModelLoadingProgress = (label: string) => {
      const average = (modelProgress.player + modelProgress.police) / 2;
      setLoadingProgress(24 + average * 52, label);
    };
    const reportModelProgress =
      (vehicle: keyof typeof modelProgress, label: string) =>
      (event: ProgressEvent<EventTarget>) => {
        if (event.lengthComputable && event.total > 0) {
          modelProgress[vehicle] = THREE.MathUtils.clamp(
            event.loaded / event.total,
            0,
            1,
          );
          updateModelLoadingProgress(label);
        }
      };
    const failSceneLoading = () => {
      loadingFailed = true;
      sceneReady = false;
      playButton.disabled = true;
      assetStatus.classList.add("has-error");
      setLoadingProgress(0, "GAME LOAD FAILED — RELOAD");
    };
    const preloadCompleteScene = async () => {
      if (preloadStarted || loaded < 2 || loadingFailed) return;
      preloadStarted = true;
      try {
        setLoadingProgress(79, "UPLOADING TEXTURES…");
        ...
        setLoadingProgress(88, "COMPILING FULL SCENE…");
        ...
        try {
          ...
          setLoadingProgress(97, "WARMING UP PURSUIT…");
          const warmupTarget = new THREE.WebGLRenderTarget(16, 16, {
            depthBuffer: true,
          });
        ...
        setLoadingProgress(100, "READY TO RUN");
        playButton.disabled = true;
      } catch (error) {
    ...
    const assetLoaded = () => {
      runWhenHostActive(() => {
        loaded++;
        if (loaded >= 2) void preloadCompleteScene();
      });
    };
    ...
    setLoadingProgress(24, "LOADING SPORTS CAR…");
    loader.load(
      ...
      (gltf) => {
        runWhenHostActive(() => {
          ...
          updateModelLoadingProgress("LOADING POLICE UNITS…");
          assetLoaded();
        });
      },
      reportModelProgress("player", "LOADING SPORTS CAR…"),
      assetLoadFailed,
    );
    loader.load(
      ...
      (gltf) => {
        runWhenHostActive(() => {
          ...
          modelProgress.police = 1;
          updateModelLoadingProgress("PREPARING FULL SCENE…");
          assetLoaded();
        });
      },
      reportModelProgress("police", "LOADING POLICE UNITS…"),
      assetLoadFailed,
    );
    ...
    const impactFlash = new THREE.Mesh(
      new THREE.IcosahedronGeometry(0.62, mobileRendering ? 0 : 1),
      impactFlashMaterial,
    );
    ...
    const showVehicleImpact = (position: THREE.Vector3) => {
      ...
      impactFlash.scale.setScalar(0.35);
      impactFlashMaterial.opacity = 0.9;
      impactFlash.visible = true;
      impactFlashLife = 0.24;
      emit(position, mobileRendering ? 12 : 18, 7.2);
    };
    ...
    const playVehicleImpact = () => {
      ...
      thump.type = "triangle";
      thump.frequency.setValueAtTime(96, when);
      thump.frequency.exponentialRampToValueAtTime(38, when + 0.14);
      thumpGain.gain.setValueAtTime(0.14, when);
      thumpGain.gain.exponentialRampToValueAtTime(0.0001, when + 0.18);
      thump.connect(thumpGain).connect(engineBus);
      thump.start(when);
      thump.stop(when + 0.2);

      const scrape = audio.createBufferSource();
      const scrapeFilter = audio.createBiquadFilter();
      const scrapeGain = audio.createGain();
      scrape.buffer = noiseBuffer;
      scrapeFilter.type = "bandpass";
      scrapeFilter.frequency.value = 720;
      scrapeFilter.Q.value = 0.72;
      scrapeGain.gain.setValueAtTime(0.085, when);
      scrapeGain.gain.exponentialRampToValueAtTime(0.0001, when + 0.16);
      scrape.connect(scrapeFilter).connect(scrapeGain).connect(engineBus);
      scrape.start(when);
      scrape.stop(when + 0.18);
    };
    const setupAudio = () => {
      if (audio) {
        syncAudioPolicy();
    ...
    const beginLevel = (nextLevel: number) => {
      if (hostPaused || !sceneReady) return;
    ...
    const animate = () => {
      ...
      if (gamePhase === "playing") {
        ...
        for (let index = 0; index < activeCopCount; index++) {
          ...
          const catchup =
            distance > 28
              ...
              : distance > 12
                ? 2.8 * grip
                : distance > POLICE_BUMPER_CONTACT_GAP + 0.65
                  ? 1.1 * grip
                  : -0.8;
          ...
          const chaseGap =
            POLICE_BUMPER_CONTACT_GAP +
            (index % 2) * 1.65 +
            Math.floor(index / 2) * 2.7;
        ...
        for (let index = 0; index < activeCopCount; index++) {
          const cop = cops[index];
          const grip = currentLevel().policeGrip;
          const bodyContact = trackBodyContact(
            progress,
            lane,
            cop.progress,
            cop.lane,
          ...
          if (bodyContact) {
            if (
              bodyContact.lateralOverlap < bodyContact.longitudinalOverlap
            ) {
              const separation = bodyContact.lateralOverlap * 0.52 + 0.025;
              const laneLimit = driveableLaneLimit();
              lane = THREE.MathUtils.clamp(
                lane + bodyContact.lateralDirection * separation * 0.56,
                -laneLimit,
                laneLimit,
              );
              cop.lane = THREE.MathUtils.clamp(
                cop.lane - bodyContact.lateralDirection * separation * 0.44,
                -laneLimit,
                laneLimit,
              );
            } else {
              cop.progress = Math.min(
                cop.progress,
                progress - PLAYER_HALF_LENGTH - COP_HALF_LENGTH + 0.04,
              );
            }

            if (cooldown <= 0) {
              const centeredContact = Math.abs(lane - cop.lane) < 0.1;
              const impactDirection = centeredContact
                ? steer === 0
                  ? index % 2 === 0
                    ? 1
                    : -1
                  : Math.sign(steer)
                : bodyContact.lateralDirection;
              lateral +=
                impactDirection *
                (3.8 +
                  Math.min(bodyContact.longitudinalOverlap, 0.6) * 2.2);
              headingOffset += impactDirection * 0.045;
              speed *= 0.9;
              cop.stun = Math.max(cop.stun, 0.16);
              cop.impactLean = -impactDirection * 0.17;
              shake = Math.max(shake, 0.72);
              cooldown = 0.48;
              setTrackPosition(
                effectPosition,
                (progress + cop.progress) * 0.5,
                (lane + cop.lane) * 0.5,
                0.82,
                levelIndex,
              );
              showVehicleImpact(effectPosition);
              playVehicleImpact();
            }
          }
          setTrackPosition(
      ...
      if (impactFlashLife > 0) {
        ...
        const flashRatio = impactFlashLife / 0.24;
        impactFlashMaterial.opacity = flashRatio * 0.88;
        impactFlash.scale.setScalar(0.35 + (1 - flashRatio) * 1.45);
        impactFlash.rotation.y += dt * 9;
        if (impactFlashLife <= 0) impactFlash.visible = false;
      }
      if (audio && musicBus) {
        const musicLevel =
          gamePhase === "playing"
      ...
      if (!gameReadyReported && sceneReady && firstFrameReported) {
        youtubePlayables.gameReady();
        gameReadyReported = true;
        playButton.disabled = hostPaused;
      }
    };
    ...
    const applyYouTubeState = (state: YouTubePlayablesState) => {
      ...
      gameShell.classList.toggle("youtube-paused", hostPaused);
      playButton.disabled =
        hostPaused || !sceneReady || !gameReadyReported || loadingFailed;
